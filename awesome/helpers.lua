...
function helpers.screenshot(action, delay)
    ...
    local maim_args = "--hidecursor -b 3"
            ...
            function(_, __, ___, exit_code)
                if exit_code == 0 then
                    naughty.notify(
                        {
                            title = "Captured",
                            message = "Saved to " .. user.screenshot_dir,
                            icon = icon
                        }
                    )
                end
    ...
    elseif action == "clipboard" then
        ...
        cmd =
            "maim " ..
            maim_args ..
                " -s /tmp/maim_clipboard && xclip -selection clipboard -t image/png /tmp/maim_clipboard && rm /tmp/maim_clipboard"
        awful.spawn.easy_async_with_shell(
            cmd,
            function(_, __, ___, exit_code)
                if exit_code == 0 then
